...
extern int main()
{
	...
	for (i = 0; i < 24 * 11; i++) {
		unsigned Ll = lion_decoding[i / 11], pos = i % 11, L, l;
	...
	printf("static const signed char pos2_encoding[11 * 11] = {\n");
	for (i = 0; i < 121; i++)
		printf("%s%2d,%s", i % 11 == 0 ? "\t" : " ", pos2_encoding[i], i % 11 == 10 ? "\n" : "");
	printf("};\n\n");
	 ...
	 */
	printf("static const unsigned char flip_op[256] = {\n");
	for (i = 0; i < 256; i++)
		printf("%s%#4x,%s", i % 16 == 0 ? "\t" : " ",
		    i ^ ((i & 0x55) << 1 | (i & 0xaa) >> 1), i % 16 == 15 ? "\n" : "");
	printf("};\n");
